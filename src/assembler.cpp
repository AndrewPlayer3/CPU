...
// Not in use yet.
std::string builder_2(std::vector<std::pair<std::string, ARG_TYPE>>& instr) {
    std::string op = instr[0].first;
    int op_bits  = op_to_int[op][0];
    //int reg_bits = op_to_int[op][1];
    int int_bits = op_to_int[op][2];
    ...
    if(instr[1].second == POINTER) {
        // We need instructions for these things...
        switch(instr[2].second) {
            case INTEGER:
                B = int_bits;
                C = 0;
                D = 0;
                ptr_value = to_int(trim(instr[2].first.substr(1, trim(instr[2].first).size() - 2)));
                is_int_arg = true;
                break;
            case REGISTER:
                B = 0;
            ...
            case STRING:
                ...
                break;
            default:
                std::cout << "builder_2 couldn't match \"" << instr[0].first << " " << instr[1].first << ", " << instr[2].first << std::endl;
                exit(1); 
        }
    }
    int inst = (op_bits << 12) | (B << 8) | (C << 4) | (D << 0);
